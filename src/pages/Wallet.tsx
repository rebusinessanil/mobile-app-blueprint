...
export default function Wallet() {
  ...
  useEffect(() => {
    const initializeWallet = async () => {
      ...
      const {
        data: profile
      } = await supabase.from("profiles").select("customer_code").eq("user_id", user.id).maybeSingle();
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`wallet-updates-${userId}`).on("postgres_changes", {
      event: "*",
      schema: "public",
      table: "user_credits",
  ...
  const fetchWalletData = async (uid: string) => {
    try {
      ...
      const {
        data: credits,
        error: creditsError
      } = await supabase.from("user_credits").select("balance, total_earned, total_spent").eq("user_id", uid).maybeSingle();
      ...
      const {
        data: txns,
        error: txnsError
      } = await supabase.from("credit_transactions").select("*").eq("user_id", uid).order("created_at", {
        ascending: false
      });
      ...
      const {
        data: rechargeData
      } = await supabase.from("credit_transactions").select("amount, created_at").eq("user_id", uid).in("transaction_type", ["earned", "admin_credit"]).order("created_at", {
        ascending: false
      }).limit(1).maybeSingle();
      if (rechargeData) {
        setLastRecharge({
      ...
      const totalSpent = txns?.filter(t => t.transaction_type === "spent" || t.transaction_type === "debit").reduce((sum, t) => sum + Math.abs(t.amount), 0) || 0;
      setBalance({
  ...
  const getTransactionIcon = (type: string) => {
    return type === "earned" || type === "admin_credit" ? <ArrowUp className="w-4 h-4" /> : <ArrowDown className="w-4 h-4" />;
  };
  const getTransactionColor = (type: string) => {
    return type === "earned" || type === "admin_credit" ? "text-green-500" : "text-red-500";
  };
  if (loading) {
  ...
  return <div className="min-h-screen bg-navy-dark flex flex-col overflow-x-hidden">
      ...
      <div className="px-4 sm:px-6 space-y-4 flex-shrink-0 max-w-screen-md mx-auto w-full">
        ...
        <Card className="gold-border bg-gradient-to-br from-primary/10 to-primary/5">
          ...
          <CardContent className="space-y-3">
            ...
            {customerCode && <div className="flex items-center gap-2 text-xs">
            ...
            <div className="flex gap-2">
              <Button onClick={() => navigate("/my-downloads")} variant="outline" size="sm" className="flex-1 border-primary/30 text-foreground hover:bg-primary/10 rounded-lg text-xs h-9">
                <Download className="w-3.5 h-3.5 mr-1.5" />
                My Downloads
              </Button>
              <Button onClick={handleTopUp} size="sm" className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-lg text-xs h-9">
      ...
      <div className="px-4 sm:px-6 flex-1 flex flex-col min-h-0 pb-24 mt-2 max-w-screen-md mx-auto w-full">
        <Card className="border-primary/20 bg-card flex-1 flex flex-col min-h-0">
          ...
          <CardContent className="flex-1 overflow-y-auto pr-2" style={{
          ...
          WebkitOverflowScrolling: 'touch'
            ...
            {transactions.length === 0 ? <div className="text-center py-12 text-muted-foreground">
                ...
                {transactions.map(txn => <div key={txn.id} className="flex items-center justify-between py-3 border-b border-border/50 last:border-0">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center ${txn.transaction_type === "earned" || txn.transaction_type === "admin_credit" ? "bg-green-500/10" : "bg-red-500/10"}`}>
                    ...
                    <span className={`font-bold text-sm ${getTransactionColor(txn.transaction_type)}`}>
                      {txn.transaction_type === "earned" || txn.transaction_type === "admin_credit" ? "+" : "-"}
                      ₹{Math.abs(txn.amount)}
                    </span>
                  </div>)}
              </div>}
