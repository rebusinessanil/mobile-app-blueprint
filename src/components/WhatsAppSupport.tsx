...
export default function WhatsAppSupport() {
  ...
  useEffect(() => {
    // Check if hidden in current session
    const hidden = sessionStorage.getItem(HIDDEN_KEY);
    if (hidden === "true") {
      setIsHidden(true);
    }

    // Listen for login events to show icon again
  ...
  return (
    <div className="fixed bottom-24 right-6 z-50 group">
      ...
      <button
        onClick={handleClose}
        className="absolute -top-1 -right-1 w-5 h-5 bg-muted rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
