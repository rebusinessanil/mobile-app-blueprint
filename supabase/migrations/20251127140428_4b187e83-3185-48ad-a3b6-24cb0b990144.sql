...
-- Explicitly deny access to anonymous (anon) role
...
DROP POLICY IF EXISTS "Deny anonymous access to all storage" ON storage.objects;

-- Explicitly deny all anonymous access to storage.objects
CREATE POLICY "Deny anonymous access to all storage"
